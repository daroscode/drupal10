/**
 * @file
 * Provides UI/UX progressive enhancements on Barrio's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.barrio.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-barrio-custom-color', changedInput.value);
    inputToSync.setAttribute('data-barrio-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-barrio-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-barrio-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes barrio theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-barrio-custom-color',
      textInput.getAttribute('data-barrio-custom-color'),
    );
  ...
  /**
   * Barrio Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.barrioColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'barrio-color-picker',
        '[data-drupal-selector="edit-bootstrap-barrio-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'barrio-color-picker',
        '[data-drupal-selector="barrio-color-picker"] input[type="text"]',
      );
